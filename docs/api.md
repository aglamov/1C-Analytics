...
Если объект раздела содержит `hasExtended: true`, после стандартных карточек
показывается кнопка второго уровня. Она выполняет запрос с параметром
`section=<section>_Расширенный` и добавляет полученные карточки в конец раздела.
Активированные дополнительные наборы повторно запрашиваются при обновлении, но
не сохраняются в кэш и после перезапуска загружаются только по новой команде пользователя.
...
| `PlanFactProgress` | Текущее представление план-факта | План и оплата по периодам |

Неизвестный тип считается нарушением контракта: такая секция отклоняется, а
интерфейс сохраняет последнюю корректную версию раздела и показывает ошибку
частичного обновления. Это не позволяет незаметно отрисовать искажённые данные.
...
- `showLegend` управляет легендой; по умолчанию она показана, кроме `GeoMap` и
  явного `PlanFactProgress`;
- `showTotal` управляет выводом агрегированного значения; значение по умолчанию
  `true`, для `GeoMap` и явного `PlanFactProgress` — `false`;
...
Явный тип `PlanFactProgress` преобразуется во внутреннее представление план-факта
