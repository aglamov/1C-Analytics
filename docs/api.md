...
`RadarMark` отклоняется, если содержит менее трёх осей или отрицательные
значения. В `ExpandableTableMark` явные `id` узлов должны быть уникальны среди
соседних элементов одного уровня; повтор считается нарушением контракта.
