...
| `Gauge` | Круговой индикатор | Доля текущего `value` от `valueMax` |
| `GeoMap` | Интерактивная карта мира | Распределение значений по странам |
...
количество. На обзорной карте отображаются 40 стран с наибольшими значениями,
полный список остаётся доступен в детализации.
...
- `widthPercent` сохраняет желаемую ширину карточки (сейчас API использует `50`
  для двухколоночной компоновки на iPad).
